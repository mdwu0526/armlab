...
def get_block_detections(imageFrame):
    ...
    #all_masks = np.logical_or(red_mask, green_mask, blue_mask, yellow_mask, orange_mask, purple_mask)
    # print(all_masks.shape)
    # all_masks = all_masks.astype(np.uint8)  #convert to an unsigned byte
    # all_masks*=255
    # cv2.imwrite("all_masks.png", all_masks)
    ...
    for mask in masks:
        ...
        all_masks*=255
    cv2.imwrite("all_masks.png", all_masks)
    #with_blocks = np.logical_and(all_masks, hsvFrame)
    with_blocks = hsvFrame
    with_blocks[~all_masks] = 0
    cv2.imwrite("with_blocks.png", with_blocks)
